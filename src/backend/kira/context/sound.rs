...
#[deprecated(since = "0.1.0-alpha.1", note = "move map to lazy_static")]
...
impl SoundContext {
    ...
    pub fn play_sound(&self, sound: Sound) {
        if let Some(mut sound_handle) = self.sound_map.get_mut(&sound) {
            match sound_handle.play(kira::instance::InstanceSettings::default()) {
                Ok(_handle) => {
                    // macroquad::prelude::debug!("Played sound {:?}", sound);
    ...
    pub fn add_sound(&self, sound: Sound, bytes: &[u8]) {
        match crate::backend::kira::from_ogg_bytes(bytes, kira::sound::SoundSettings::default()) {
            Ok(sound_handle) => {
                match super::AUDIO_CONTEXT.lock().as_mut() {
                    Some(context) => {
